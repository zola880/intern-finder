...
const Navbar = () => {
  ...
  if (user) {
    if (user.role === "EMPLOYER" || user.role === "UNIVERSITY_ADMIN" || user.role === "ADMIN") {
      navLinks.push({ name: "Company", path: "/company" });
    } else {
          ...
          <div className="flex items-center gap-3">
            ...
            {user ? (
              ...
              <div className="flex items-center gap-2">
                <Link
                  to="/register"
                  className="px-4 py-2 border border-indigo-600 text-indigo-600 rounded-lg text-sm font-semibold transition hover:bg-indigo-50 dark:hover:bg-zinc-800"
                >
                  Register
                </Link>
                <Link
